...
def train(args, train_dataset, model, tokenizer):
    ...
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=False, ascii=True)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_eval_file, "w") as writer:
            ...
            writer.write(
                "Confusion matrix is:\n %s"
                % str(confusion_matrix(y_true=out_label_ids, y_pred=preds))
            )
...
def main():
    ...
    parser.add_argument(
        "--save_steps", type=int, default=1000, help="Save checkpoint every X updates steps."
    )
    parser.add_argument(
