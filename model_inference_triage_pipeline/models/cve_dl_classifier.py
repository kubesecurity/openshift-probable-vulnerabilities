...
class CVEClassifier:
    ...
    def __init__(self, tokenizer_path, model_weights_path,
        ...
        self.MAX_FEATURES = 600000 if not max_features else max_features
        ...
        _logger.info('Loading CVE Model Tokenizer Vocabulary')
    ...
    def build_model_architecture(self, gru_units=32):
        _logger.info('Building CVE Model Architecture')
    ...
    def load_model_weights(self, model_weights_path=None):
        _logger.info('Loading CVE Model Weights')
    ...
    def get_model(self):
        if not self.MODEL:
            raise ModelNotBuiltException(
                "CVE Model doesn't exist. Please build model first")
        else:
            return self.MODEL
